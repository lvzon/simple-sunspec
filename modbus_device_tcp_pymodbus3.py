...
# Interact with a modbus-device over TCP using Pymodbus 3.x
...
class ModbusClientTCP():
    ...
    def read (self, reg_start, registers = 1):
        rr = self.client.read_holding_registers(reg_start, registers, slave = self.dev_id)
        if rr.isError():
            return None
        return rr.registers
    ...
    def write (self, reg_start, values):
        ...
        if count == 1:
            ...
            wr = self.client.write_register(reg_start, values[0], slave = self.dev_id)
        elif count > 1:
            # Use function code 0x10 to write multiple registers
            wr = self.client.write_registers(reg_start, registers, slave = self.dev_id)
        else:
            return 0
